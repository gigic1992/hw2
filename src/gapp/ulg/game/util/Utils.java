...
public class Utils {
    ...
    public static <P> Board<P> UnmodifiableBoard(Board<P> b) {
        if(b==null){
            throw new NullPointerException("la board non può essere nulla");
        }
        return new VisBoard<>(b);
    }
    ...
    public static <P> GameRuler<P> play(GameFactory<? extends GameRuler<P>> gf, Player<P>...pp) {
        if (gf==null){throw new NullPointerException("la gamefactory non può essere nulla");}
        for (int i =0;i<pp.length;i++){
            if(pp[i]==null){throw new NullPointerException("nessun giocatore può essere nullo");}
        }
        String[] nomi_gioc=new String[0];
        for (Player i : pp){
            nomi_gioc= Arrays.copyOf(nomi_gioc,nomi_gioc.length+1);
            nomi_gioc[nomi_gioc.length-1]=i.name();
        }
        gf.setPlayerNames(nomi_gioc);
        GameRuler<P> gioco= gf.newGame();
        for (Player i : pp){
            i.setGame(gioco);
        }
        while(gioco.result()==-1){gioco.move(pp[gioco.turn()-1].getMove());
