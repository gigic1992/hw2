    ...
    default Set<Move<P>> validMoves(Pos p) {
        ...
        if(getBoard().get(p)!=null){
            for (Move i:validMoves()){
                if(i.getKind().equals(Move.Kind.ACTION))
                    if((((Action) i.getActions().get(0)).getKind().equals(Action.Kind.MOVE) ||
                            ((Action) i.getActions().get(0)).getKind().equals(Action.Kind.SWAP) ||
                            ((Action) i.getActions().get(0)).getKind().equals(Action.Kind.JUMP)) &&
                            ((Action) i.getActions().get(0)).getPiece().equals(getBoard().get(p))) {
                        luigi.add(i);
                    }
            }
        }
        if(getBoard().get(p) == null) {
            for (Move i : validMoves()){
                if(i.getKind().equals(Move.Kind.ACTION)) {
                    if(((Action) i.getActions().get(0)).getKind().equals(Action.Kind.ADD) &&
                            (((Action) i.getActions().get(0)).getPos().equals(Collections.singletonList(p)))) { luigi.add(i); }
                }
            }
        }
        return Collections.unmodifiableSet(luigi);
    }
