...
public interface Board<P> {
    ...
    default boolean isPos(Pos p) {
        if (p==null){throw new NullPointerException("posizione non può essere nulla");}
    ...
    default Set<Pos> get() {Set<Pos> a=new HashSet<>();
        for(Pos m : positions()){
            if(get(m) != null){a.add(m);}
        }
    ...
    default Set<Pos> get(P pm) {
        if (pm == null) {
            throw new NullPointerException("un modello di pezzo non può essere nullo");}
        Set<Pos> a=new HashSet<>();
        for (Pos n: positions()){
            if(Objects.equals(get(n), pm)){a.add(n);}
        }
        return Collections.unmodifiableSet(a);
    }
    ...
    default void put(P pm, Pos p, Dir d, int n) {
        ...
        if(pm==null||p==null||d==null){throw new NullPointerException("posizione, direzione e numero di posizioni non possono essere nulli.");}
        if(n<=0){throw new IllegalArgumentException("numero posizioni non può essere nullo.");}
        Pos rif=p;
        List<Pos> lstTemp = new ArrayList<>();
        for(int i = 0; i < n-1; i++) {
            rif=adjacent(rif,d);
            if(rif==null){throw new IllegalArgumentException("la posizione della linea specificata non è nella board");}
            lstTemp.add(rif);
        }
        put(pm, p);
        for(Pos p1 : lstTemp) { put(pm, p1); }
    }
}
